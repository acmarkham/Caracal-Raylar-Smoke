...
impl VedbaSensor {
    pub const fn new(config: VedbaConfig) -> Self {
        Self {
            config,
            gravity: None,
            last_sequence: None,
    ...
    pub(crate) fn evaluate<const SENSORS: usize>(
        ...
        CompositeEvaluation::Updated(CompositeSample {
            value: SensorValue::DynamicAccelerationMg(vector_magnitude(
...
mod tests {
    ...
    #[test]
    fn vedba_uses_vector_magnitude_after_static_filter() {
        let mut sensor = VedbaSensor::new(VedbaConfig {
            static_filter_alpha_permille: 100,
            ..VedbaConfig::new(SensorId(2), SensorId(1))
        });
        assert_eq!(sensor.dynamic_components([0, 0, 1_000]), [0, 0, 0]);
        let dynamic = sensor.dynamic_components([300, 400, 1_000]);
        assert_eq!(dynamic, [270, 360, 0]);
        assert_eq!(vector_magnitude(dynamic[0], dynamic[1], dynamic[2]), 450);
    }
}
