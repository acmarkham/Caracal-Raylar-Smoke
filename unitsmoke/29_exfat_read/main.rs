...
async fn main(_spawner: Spawner) -> ! {
    ...
    info!("SDMMC1 exFAT root directory test started");
    run_exfat_read(sd).await
}

async fn run_exfat_read(mut sd: SdCard<'static>) -> ! {
    sd.power.set_high();
    ...
    loop {
        match dir.next_entry(&mut fs).await {
            Ok(Some(entry)) => {
            ...
            Err(e) => {
                error!(
                    "Directory iteration failed after {} entries: {}",
                    entry_count, e
                );
                pending_forever().await;
            }
    ...
    info!(
        "exFAT root directory listing complete: entries={}",
        entry_count
    );
    pending_forever().await
}
...
impl BlockDevice<BLOCK_BYTES> for PartitionedSd<'_, '_> {
    ...
    async fn write(
        &mut self,
        _block_address: u32,
        _data: &[Aligned<Self::Align, [u8; BLOCK_BYTES]>],
    ) -> Result<(), Self::Error> {
        Err(ExfatSdError::ReadOnly)
    }
