...
const TCXO_TUNE_V3P0: u8 = 0x06;
const TCXO_STARTUP_DELAY_TICKS: u32 = 164;
const EBYTE_RFSW_ENABLE_MASK: u8 = 0x03;
const EBYTE_RFSW_RX: u8 = 0x00;
const EBYTE_RFSW_SUBGHZ_TX_LP: u8 = 0x01;
const EBYTE_RFSW_SUBGHZ_TX_HP: u8 = 0x02;
const EBYTE_RFSW_2P4GHZ: u8 = 0x03;
...
async fn configure_radio(spi: &mut RadioSpi, cs: &mut Output<'static>, busy: &Input<'static>) {
    ...
    // Conservative default PA config from the lr11xx crate plus +10 dBm / 48 us ramp.
    lr11xx_write(
        spi,
        cs,
        busy,
    ...
    .await;
    lr11xx_write(spi, cs, busy, Opcode::SetTxParams, &[0x0A, 0x02]).await;
...
async fn configure_rf_switches(spi: &mut RadioSpi, cs: &mut Output<'static>, busy: &Input<'static>) {
    ...
    // 00 = RX, 01 = sub-GHz TX low-power, 10 = sub-GHz TX high-power, 11 = 2.4 GHz path.
    // On LR11xx, the high-frequency frontend state is configured through the "wifi" slot.
    lr11xx_write(
        spi,
        cs,
        busy,
...
async fn enable_tcxo(spi: &mut RadioSpi, cs: &mut Output<'static>, busy: &Input<'static>) {
    ...
    lr11xx_write(
        ...
        busy,
        Opcode::SetTcxoMode,
        &tcxo_mode_bytes(TCXO_TUNE_V3P0, TCXO_STARTUP_DELAY_TICKS),
    )
    .await;
...
fn rf_switch_config_bytes(
    ...
    [enable, standby, rx, tx, tx_hp, 0x00, gnss, wifi]
}
