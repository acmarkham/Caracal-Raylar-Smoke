...
async fn i2c_task(sens_i2c: SensI2C<'static>, mut led: Output<'static>) -> ! {
    ...
    match i2c.blocking_write(LIS2MDL_ADDR, &[REG_CTRL1, ctrl1]) {
        Ok(_) => info!("Started"),
        Err(e) => info!("CONFIG:LIS2MDL error: {:?}", e),
    }
    // set to continuous mode
    unwrap!(i2c.blocking_write(LIS2MDL_ADDR, &[REG_CTRL3, 0x00],));

    loop {
        led.set_high();
        let mut raw = [0u8; 6];
        match i2c.blocking_write_read(LIS2MDL_ADDR, &[REG_OUT_X_L | 0x80], &mut raw) {
            Ok(_) => {
                let x = i16::from_le_bytes([raw[0], raw[1]]);
                let y = i16::from_le_bytes([raw[2], raw[3]]);
                let z = i16::from_le_bytes([raw[4], raw[5]]);
                info!("MAG x={} y={} z={}", x, y, z);
            }
            Err(e) => info!("LIS2MDL error: {:?}", e),
        }
