// Test of the accelerometer on the SensI2C bus. This is a smoke test to verify that the I2C bus is working and that the accelerometer is responding with data.

// Activity: This test will read the accelerometer data from the LIS2HH12 sensor, and print the X, Y, and Z acceleration values to the console. The sys_main_red led will be turned on when the I2C bus is being accessed, and turned off when the bus is idle. The sys_main_green led will blink to indicate that the test is running.
...
async fn i2c_task(sens_i2c: SensI2C<'static>, mut led: Output<'static>) -> ! {
    ...
    const WHO_AM_I_EXPECTED: u8 = 0x41;

    // Check whoami
    match i2c.blocking_write_read(LIS2HH12_ADDR, &[REG_WHO_AM_I], &mut whoami) {
